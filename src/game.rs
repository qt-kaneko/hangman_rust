use std::{collections::BTreeSet, fmt::Display, time::{SystemTime, UNIX_EPOCH}};
use crate::{splitmix64::SplitMix64, wrap::{wrap, wrap_once}};
...
impl Game {
  ...
  pub fn won(&self) -> bool {
    self
      .word()
      .chars()
      .all(|c| self.guesses.contains(&c))
  }

  pub fn next_word(&mut self) {
    self.current = (self.random.next_u64() % self.dictionary.len() as u64) as usize;
  ...
  pub fn hint(&mut self) {
    let available: Vec<char> = self
      .word()
      .chars()
      .filter(|c| !self.guesses.contains(c))
      .collect();

    if let Some(letter) = available.get((self.random.next_u64() % available.len() as u64) as usize) {
      self.guess(*letter);
    };
  }
...
impl Display for Game {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let word = self
      .word()
      .chars()
      .map(|c| if self.guesses.contains(&c) || self.state == State::Lost { c } else { '_' })
